...
class FormWTFAddFilm(FlaskForm):
    ...
    """
    nom_film_regexp = r'^[\w.-]+@[\w.-]+\.\w+$'
    nom_film_add_wtf = StringField("Email ", validators=[Length(min=2, max=200, message="min 2 max 200"),
                                                         Regexp(nom_film_regexp, message="Email invalide")
                                                         ])
    prenom_film_add_wtf = StringField("Mot de passe ", validators=[Length(min=2, max=200, message="min 2 max 200")])

    submit = SubmitField("Enregistrer credential")
